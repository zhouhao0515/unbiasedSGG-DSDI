...
def gener_causal_root(num_objs):
    causal_root = []
    for idx in num_objs:
        causal_root.append(ArbitraryTree(idx, 1.0, 0, is_root=True))
    return causal_root

def gener_causal_vc_forest(vc_forest,causal_root):
    causal_vc_forest = []
    assert len(vc_forest)==len(causal_root)
    for i in range(len(causal_root)):
        causal_vc_root = causal_root[i].generate_bi_tree()
        vc_forest[i].is_root = False
        causal_vc_root.add_left_child(vc_forest[i])
        causal_vc_forest.append(causal_vc_root)
    return causal_vc_forest
...
class BiTree(BasicBiTree):
    def __init__(self, idx, node_score, label, box, is_root=False):
        ...
        self.box = box.view(-1) if box is not None else None
